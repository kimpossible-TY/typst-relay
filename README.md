...
[kimpossible-TY/tinymist](https://github.com/kimpossible-TY/tinymist) is the development repository, and `main` is the integration branch. The earlier separate Flow repository is retained as historical storage.
...
README is generated from `docs/tinymist/flow.typ`. Edit that source and regenerate with `node scripts/link-docs.mjs --readme-only` after building the `typlite` binary. See [AGENTS.md](AGENTS.md) for repository conventions.
